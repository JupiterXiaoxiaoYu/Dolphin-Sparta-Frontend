...
async function getTask(taskid, d_state) {
    const queryParams = {
        id: taskid,
        tasktype: "Prove",
        taskstatus: d_state,
...
async function getTaskWithTimeout(taskId, timeout) {
    return Promise.race([getTask(taskId, "Done"), new Promise((_, reject) => setTimeout(() => reject(new Error('load proof task Timeout exceeded')), timeout))
    ]);
}
export async function getWithdrawEventParameters(proxy, receipt) {
    let r = [];
    try {
        ...
        if (record) {
            ...
            if (data0.proof.length == 0) {
                ...
                if (data1.status === "DryRunFailed" || data1.status === "Unprovable") {
                    ...
                    while (1)
                        ; //This is serious error, while loop to trigger manual review.
                    return -1;
                }
                else {
